...
# requires careful setup. The _prompt() function encapsulates all requirements:
#
...
# 3. Uses explicit create_input()/create_output() - ensures proper terminal raw mode
# 4. Creates fresh objects per call - Rich output between prompts disrupts terminal state
#
# Without ALL FOUR, escape sequences get printed as garbage instead of being
# interpreted as key presses (e.g., ctrl+arrow prints "^[[1;5D" instead of moving
# by word). The console.print() before the actual prompt was an important piece,
# but draining stdin is also required to handle late-arriving CPR responses.
...
def _prompt(instruction: str, prompt_text: str = "Your response: ") -> str:
    ...
    Args:
        instruction: Text to print before the prompt (e.g., "Enter 'y' to accept...").
                     This is REQUIRED to flush Rich's terminal state before prompting.
        prompt_text: The actual prompt string shown on the input line.

    Returns:
        The user's input string.

    This function encapsulates four critical requirements for prompt_toolkit + Rich:
    ...
    3. Explicit create_input()/create_output() - ensures proper terminal raw mode
    4. Fresh objects per call - Rich output between prompts disrupts terminal state

    Without ALL FOUR, escape sequences appear as literal garbage characters:
    - ctrl+left/right arrow: prints "^[[1;5D" instead of moving cursor by word
    - cmd+v (paste): prints escape sequences instead of pasting content
    - Window resize/maximize: prints "^[[128;1R" cursor position reports
    """
    # Drain any pending terminal input (late CPR responses, etc.)
    _drain_stdin()

    # Print instruction to flush Rich's terminal state before prompt_toolkit takes over
    console = Console()
