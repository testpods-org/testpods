...
def run_proof_generation(
    ...
    *,
    ...
    # Parse showcase spec - FAIL if invalid YAML (this is a fatal error)
    try:
        showcase_spec = parse_showcase_section(plan_path)
        if showcase_spec is None:
            # Section exists but is effectively empty (no YAML block or empty YAML)
            log_and_print(
                logger,
                "Showcase section is empty (skipping proof generation)",
                print_prefix="  [Info] ",
            )
            print("PROOF_SKIPPED: empty showcase section")
            return True
