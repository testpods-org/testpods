...
def main() -> None:
    ...
    if args.dry_run:
        print("[DRY RUN] Run proof generation orchestration script")
        print("[DRY RUN] Usage: run_proof.py --flow-dir <path> --plan <path>")
        print("[DRY RUN] Would spawn proof generation agents to:")
        print("[DRY RUN]   1. Parse showcase requirements from plan")
        print("[DRY RUN]   2. Run CLI demos and capture evidence")
        print("[DRY RUN]   3. Generate narrative proof document")
        print("[DRY RUN]   4. Fact-check claims with evidence citations")
        return
