...
def run_learnings(flow_dir: Path, plan_file: str) -> tuple[str, bool, str | None]:
    ...
    Returns:
        Tuple of (task_name, success, skip_reason).
        skip_reason is always None for learnings (not currently skippable).
    """
    ...
    if result.returncode != 0:
        ...
        return "learnings", False, None

    return "learnings", True, None


def run_proof(flow_dir: Path, plan_file: str) -> tuple[str, bool, str | None]:
    ...
    Returns:
        Tuple of (task_name, success, skip_reason).
        skip_reason is None if proof was generated, or a string if skipped.
    """
    ...
    # Check if proof was skipped (look for PROOF_SKIPPED marker in output)
    skip_reason = None
    for line in result.stdout.splitlines():
        if line.startswith("PROOF_SKIPPED:"):
            skip_reason = line.split(":", 1)[1].strip()
            break

    return "proof", True, skip_reason
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        for future in as_completed(futures):
            task_name, task_success, skip_reason = future.result()
            if task_success:
                if skip_reason:
                    console.print(f"  [green]OK[/green] {task_name} [dim]({skip_reason})[/dim]")
                else:
