...
**Agent:** step-03-reviewer | **Completed:** {timestamp}
...
The previous agent (step-03-builder-retry-1) completed their work.
Their report is at: `/Users/henrik/git/henrik/testpods_project/testpods/flows/create-container-spec-builder/step-3/agents/step-03-builder-retry-1/report.complete.md`
...
Write regular progress updates to: `/Users/henrik/git/henrik/testpods_project/testpods/flows/create-container-spec-builder/step-3/agents/step-03-reviewer/report.md`

Structure your report using this template:

```markdown
# Step 3 status: [Completed | Partially completed | Failed]
...
- Write your completed report to: `/Users/henrik/git/henrik/testpods_project/testpods/flows/create-container-spec-builder/step-3/agents/step-03-reviewer/report.md`
...
- [✅] Step 1: Create ProbeSpec Fluent Builder
- [✅] Step 2: Create ContainerSpec Fluent Builder
- [❌] Step 3: Add Unit Tests for ContainerSpec and ProbeSpec <-- YOU ARE HERE
- [❌] Step 4: Update GenericTestPod to Use ContainerSpec

### Current Step (Full Content)

### Status: ❌ | Step 3: Add Unit Tests for ContainerSpec and ProbeSpec

#### Step 3 Purpose

Ensure the new builders work correctly and provide documentation through test examples. Tests serve as executable documentation for the API.

#### Step 3 Description

Create `ContainerSpecTest.java` in `core/src/test/java/org/testpods/core/pods/builders/`:

**Test cases to implement:**

1. **Basic container building:**
   - Build container with name, image, and port
   - Verify all fields are set correctly

2. **Environment variables:**
   - Add multiple env vars with `withEnv()`
   - Verify insertion order is preserved
   - Test ConfigMap reference with `withEnvFrom()`
   - Test Secret reference with `withSecretEnv()`
...
4. **Escape hatch:**
   - Use `customize()` to set imagePullPolicy
   - Use `customize()` to set securityContext
   - Verify customizer is applied

5. **Validation:**
   - Verify build fails without name (NullPointerException)
   - Verify build fails without image (NullPointerException)

6. **ProbeSpec tests:**
   - Build each probe type (tcpSocket, httpGet, httpsGet, exec)
   - Verify timing defaults
   - Verify timing overrides

**Use AssertJ** for assertions (already in project dependencies).
