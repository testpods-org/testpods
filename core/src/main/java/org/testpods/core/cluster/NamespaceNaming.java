 ...
 *
 * <p>Namespace names follow the pattern: {@code testpods-{context}-{suffix}}
 *
 * <p>This class is designed for extension by JUnit test extensions which can provide context-aware
 * naming (e.g., including test class name).
 *
...
public final class NamespaceNaming {
  ...
  public static String generate(String context) {
    ...
    if (available < 0) {
      ...
      int maxContextLen =
          MAX_NAMESPACE_LENGTH - PREFIX.length() - SUFFIX_LENGTH - 2; // 2 for dashes
   ...
   *
   * <p>Pattern: {@code testpods-{classname}-xxxxx}
   *
   * <p>This is intended for JUnit extensions to use:
   *
   * <pre>{@code
   * TestPodDefaults.setNamespaceNameSupplier(
   ...
   *
   * <p>K8s namespace names must:
   *
   * <ul>
   *   <li>Be lowercase
   *   <li>Contain only alphanumeric characters and hyphens
   *   <li>Start and end with alphanumeric character
   * </ul>
   */
