 ...
 *
 * <p>Kubernetes has its own readiness probe mechanism, but sometimes tests need additional waiting
 * logic (e.g., wait for specific log message).
 *
 * <p>Built-in strategies:
 *
 * <ul>
 *   <li>{@link #forReadinessProbe()} - Trust K8s readiness probe
 *   <li>{@link #forLogMessage(String)} - Wait for log output
 *   <li>{@link #forPort(int)} - Wait for TCP port
 *   <li>{@link #forHttp(String, int)} - Wait for HTTP endpoint
 *   <li>{@link #forCommand(String...)} - Wait for command to succeed
 *   <li>{@link #allOf(WaitStrategy...)} - Combine multiple strategies
 * </ul>
 *
 * <p>Example:
 *
...
public interface WaitStrategy {
   ...
   *
   * <p>This is the default for most infrastructure pods. It trusts the pod's configured readiness
   * probe and polls the K8s API until the pod reports ready.
   *
   * <p>Default timeout: 2 minutes
   */
   ...
   *
   * <p>Uses regex matching against the container logs.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   *
   * <p>Useful when a message appears multiple times during startup and you need to wait for a
   * specific occurrence.
   *
   * @param regex The regex pattern to match
   ...
   *
   * <p>Attempts to connect to the pod's external endpoint on the specified port.
   *
   * <p>Default timeout: 1 minute
   *
   ...
   *
   * <p>By default, considers 2xx status codes as success.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   *
   * <p>The command is executed inside the container using exec.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   *
   * <p>Strategies are executed in order. The total timeout is shared among all strategies.
   *
   * <p>Example:
   *
   * <pre>{@code
