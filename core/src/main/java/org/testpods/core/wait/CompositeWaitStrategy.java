 ...
 * <p>
 * Strategies are executed sequentially in the order provided. The total timeout
 * is shared across all strategies - each strategy gets the remaining time after
 * the previous strategies complete.
 * <p>
 * If any strategy times out or fails, the entire composite fails with a message
 * indicating which strategy failed.
 * <p>
 * Example usage:
 * <pre>{@code
 ...
 * }</pre>
 * <p>
 * Default timeout: 5 minutes (shared across all strategies)<br>
...
public class CompositeWaitStrategy implements WaitStrategy {
    ...
    private CompositeWaitStrategy(List<WaitStrategy> strategies, Duration timeout, Duration pollInterval) {
        ...
        for (int i = 0; i < strategies.size(); i++) {
            ...
            if (remaining <= 0) {
                throw new IllegalStateException(String.format(
                    ...
                    "(%s) for pod '%s'",
                    formatDuration(timeout), i + 1, strategies.size(),
                    strategy.getClass().getSimpleName(), pod.getName()));
            }

            // Apply the remaining time as the strategy's timeout
            WaitStrategy timedStrategy = strategy
                .withTimeout(Duration.ofMillis(remaining))
