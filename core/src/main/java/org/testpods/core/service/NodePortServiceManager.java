 ...
 *
 * <p>NodePort services expose the service on each node's IP at a static port. This allows external
 * access to the service via {@code <NodeIP>:<NodePort>}.
 *
 * <p>This is appropriate for:
 *
 * <ul>
 *   <li>External access for testing (connecting from test code to pods)
 *   <li>Development and testing environments
 *   <li>When LoadBalancer is not available
 * </ul>
 */
public class NodePortServiceManager implements ServiceManager {
   ...
   *
   * <p>If not set, Kubernetes will assign a port from the node port range (default 30000-32767).
   *
  ...
  public Service create(ServiceConfig config) {
    ...
    LOG.debug(
        "Created NodePort service: {}/{} (nodePort: {})",
        config.namespace(),
        config.name(),
        getNodePort());
    return service;
   ...
   *
   * <p>This is only available after the service has been created.
   *
