 ...
 *
 * <ul>
 *   <li>Sensible defaults - just {@code new MongoDBPod()} works
 *   <li>Version selection via {@link #withVersion(String)}
 *   <li>Authentication via {@link #withCredentials(String, String)}
 *   <li>Database selection via {@link #withDatabase(String)}
 *   <li>Connection string helpers for both internal and external access
 *   <li>Client factory for convenience
 * </ul>
 *
 * <p>Example:
 *
...
public class MongoDBPod extends StatefulSetPod<MongoDBPod> {
  ...
  /**
   * Configure authentication credentials. Sets MONGO_INITDB_ROOT_USERNAME and
  ...
  /**
   * Configure as a replica set member. Note: For testing, single-node replica sets are supported.
   */
  ...
  /**
   * Create a MongoDB client connected to this instance. Uses external connection string (for test
   * code).
   */
  ...
  protected Container buildMainContainer() {
    ...
    builder.addToEnv(
        new EnvVarBuilder().withName("MONGO_INITDB_DATABASE").withValue(database).build());
