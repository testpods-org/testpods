 ...
 * <p>
 * This is returned by {@link TestPod#exec(String...)} and contains
 * the exit code, stdout, and stderr of the executed command.
 * <p>
 * Example usage:
 * <pre>{@code
...
public record ExecResult(
     ...
     * <p>
     * Useful when you just want all output regardless of stream.
     *
    ...
    public String toString() {
        return String.format("ExecResult[exitCode=%d, stdout=%d chars, stderr=%d chars]",
    ...
    public static class ExecFailedException extends RuntimeException {
        ...
        public ExecFailedException(ExecResult result) {
            super(String.format("Command failed with exit code %d: %s",
        ...
        public ExecFailedException(String message, ExecResult result) {
            super(String.format("%s (exit code %d): %s",
                message, result.exitCode(), result.stderr().isEmpty() ? result.stdout() : result.stderr()));
            this.result = result;
        }
