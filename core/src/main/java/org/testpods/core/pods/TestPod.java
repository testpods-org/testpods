...
/**
 * Primary abstraction for a test workload in Kubernetes. Represents a running component that tests
 * can interact with.
 *
 * <p>This is the "Testcontainers equivalent" interface - like {@code Container} in Testcontainers,
...
public interface TestPod<SELF extends TestPod<SELF>> {
  ...
  /**
   * Set the name for this pod and its associated resources. This becomes the K8s resource name and
  ...
  /**
   * Add an init container that runs before the main container starts. Init containers run
   * sequentially and must complete successfully.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * Add a sidecar container that runs alongside the main container. Sidecars share the pod's
   * network and can share volumes.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * Customize the Pod spec directly using Fabric8 builders. This is the escape hatch for advanced
   * K8s configuration not exposed through the high-level API.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * Configure how to wait for this pod to be ready. Defaults vary by pod type (e.g., readiness
   * probe for databases).
   */
  ...
  /**
   * Start the pod and wait for it to be ready. Creates all necessary K8s resources (workload,
   * service, configmaps, etc.).
   */
  void start();
  ...
  /** Get the cluster-internal DNS hostname. Format: {name}.{namespace}.svc.cluster.local */
  ...
  /**
   * Get the external hostname for test code to connect. Depends on cluster's ExternalAccessStrategy
   * (port-forward, nodeport, etc.).
   */
  ...
  /**
   * Publish connection properties to a shared PropertyContext. Called automatically after the pod
   * starts.
   *
   * <p>Implementations should publish both internal and external connection details using the
