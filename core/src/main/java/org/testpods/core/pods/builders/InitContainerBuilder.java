...
/** Simplified builder for init containers. Hides Fabric8 complexity for common use cases. */
public class InitContainerBuilder {
  ...
  public InitContainerBuilder withVolumeMount(String name, String mountPath) {
    this.volumeMounts.add(new VolumeMountBuilder().withName(name).withMountPath(mountPath).build());
    return this;
  }
  ...
  public io.fabric8.kubernetes.api.model.Container build() {
    var builder =
        new io.fabric8.kubernetes.api.model.ContainerBuilder().withName(name).withImage(image);
