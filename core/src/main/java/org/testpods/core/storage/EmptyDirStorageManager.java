 ...
 *
 * <p>EmptyDir volumes are created when a pod is assigned to a node and exist as long as the pod
 * runs on that node. Data is lost when the pod is removed.
 *
 * <p>Use this for:
 *
 * <ul>
 *   <li>Scratch space for computations
 *   <li>Temporary file storage
 *   <li>Sharing data between containers in a pod
 * </ul>
 */
public class EmptyDirStorageManager implements StorageManager {
  ...
  public List<Volume> getVolumes() {
    VolumeBuilder builder =
        new VolumeBuilder().withName(volumeName).withNewEmptyDir().endEmptyDir();

    if (sizeLimit != null) {
      builder.editEmptyDir().withSizeLimit(new Quantity(sizeLimit)).endEmptyDir();
    }

    if (medium != null) {
      builder.editEmptyDir().withMedium(medium).endEmptyDir();
    }
  ...
  public List<VolumeMount> getMountsFor(String containerName) {
    return List.of(new VolumeMountBuilder().withName(volumeName).withMountPath(mountPath).build());
  }
