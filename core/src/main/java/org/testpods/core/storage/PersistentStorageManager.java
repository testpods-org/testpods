 ...
 *
 * <p>For StatefulSets, provides PVC templates that are automatically created. For Deployments,
 * creates standalone PVCs.
 */
public class PersistentStorageManager implements StorageManager {
  ...
  public void createPvcs(String namespace, KubernetesClient client) {
    ...
    for (PersistentVolumeClaim pvc : getPvcTemplates()) {
      PersistentVolumeClaim withNs =
          new PersistentVolumeClaimBuilder(pvc)
              .editMetadata()
              .withNamespace(namespace)
              .endMetadata()
              .build();

      client.persistentVolumeClaims().inNamespace(namespace).resource(withNs).create();
  ...
  public void deletePvcs(String namespace, KubernetesClient client) {
    client.persistentVolumeClaims().inNamespace(namespace).withName(volumeName).delete();
