...
class ServiceConfigTest {
    ...
    void shouldRejectNullName() {
        ...
        try {
            assertThatThrownBy(() -> new ServiceConfig(
                null, "ns", 80, Map.of(), Map.of(), List.of(), client
    ...
    void shouldRejectNullNamespace() {
        ...
        try {
            assertThatThrownBy(() -> new ServiceConfig(
                "svc", null, 80, Map.of(), Map.of(), List.of(), client
            )).isInstanceOf(NullPointerException.class)
    ...
    void shouldRejectInvalidPort() {
        ...
        try {
            assertThatThrownBy(() -> new ServiceConfig(
                "svc", "ns", 0, Map.of(), Map.of(), List.of(), client
            )).isInstanceOf(IllegalArgumentException.class)
              .hasMessageContaining("port");

            assertThatThrownBy(() -> new ServiceConfig(
                "svc", "ns", -1, Map.of(), Map.of(), List.of(), client
            )).isInstanceOf(IllegalArgumentException.class);

            assertThatThrownBy(() -> new ServiceConfig(
                "svc", "ns", 70000, Map.of(), Map.of(), List.of(), client
            )).isInstanceOf(IllegalArgumentException.class);
        } finally {
            client.close();
        }
    ...
    void shouldRejectNullClient() {
        assertThatThrownBy(() -> new ServiceConfig(
            "svc", "ns", 80, Map.of(), Map.of(), List.of(), null
        )).isInstanceOf(NullPointerException.class)
    ...
    void shouldSupportCustomizers() {
        ...
        try {
            UnaryOperator<ServiceBuilder> customizer = builder -> builder
                .editSpec().withType("LoadBalancer").endSpec();
