 ...
 * tests that actually create StatefulSets should be in a separate test class.
 */
class StatefulSetManagerTest {
  ...
  void shouldSupportFluentServiceNameConfiguration() {
    StatefulSetManager configured = new StatefulSetManager().withServiceName("my-headless-service");
  ...
  void shouldSupportFluentPvcTemplatesConfiguration() {
    ...
    StatefulSetManager configured = new StatefulSetManager().withPvcTemplates(List.of(pvc));
  ...
  void shouldSupportCombinedFluentConfiguration() {
    PersistentVolumeClaim pvc =
        new PersistentVolumeClaimBuilder().withNewMetadata().withName("data").endMetadata().build();

    StatefulSetManager configured =
        new StatefulSetManager().withServiceName("my-service").withPvcTemplates(List.of(pvc));
  ...
  void shouldHandleNullPvcTemplates() {
    ...
    assertThatCode(() -> manager.withPvcTemplates(null)).doesNotThrowAnyException();
  }
